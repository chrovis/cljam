...
(defn- map-to-chunks [f chunks]
  (letfn [(step [chunks]
            (lazy-seq
             (when-let [s (seq chunks)]
               (chunk-cons (f (chunk-first s))
                           (step (chunk-next s))))))]
    (step chunks)))

(defn read-blocks-into-chunks
  ([rdr chunk-size]
   (map-to-chunks decoder/decode-blocks-into-chunks (read-to-finish rdr chunk-size)))
  ([^BAMReader rdr chr start end chunk-size]
   (let [bai @(.index-delay rdr)]
     (if (= chr "*")
       (do (.seek ^BGZFInputStream (.reader rdr) (ffirst (bai/get-unplaced-spans bai)))
           (read-blocks-into-chunks rdr chunk-size))
       (let [refs (.refs rdr)]
         (->> (bai/get-spans bai (refs/ref-id refs chr) start end)
              (mapcat (fn [[begin finish]]
                        (->> (read-to-finish rdr begin finish chunk-size)
                             (map-to-chunks #(decoder/decode-blocks-into-chunks % start end)))))))))))
