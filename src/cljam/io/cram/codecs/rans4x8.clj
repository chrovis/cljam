...
(defn- encode1* ^long [^ByteBuffer in ^objects syms ^ByteBuffer out]
  (let [raw-size (.remaining in)
        ...
        states (int-array 4 RANS_BYTE_L)
        indices (doto (int-array 4 0)
                  (aset 0 (- q 2))
                  (aset 1 (- (* 2 q) 2))
                  (aset 2 (- (* 3 q) 2))
                  (aset 3 (- raw-size 2)))
        out' (.slice out)
        l0 (if (>= (inc (aget indices 0)) 0) (bit-and 0xff (.get in (inc (aget indices 0)))) 0)
        l1 (if (>= (inc (aget indices 1)) 0) (bit-and 0xff (.get in (inc (aget indices 1)))) 0)
        l2 (if (>= (inc (aget indices 2)) 0) (bit-and 0xff (.get in (inc (aget indices 2)))) 0)
        l3 (loop [l3 (bit-and 0xff (.get in (dec raw-size)))]
             (let [i3 (aget indices 3)]
               (if (and (> i3 (- (* 4 q) 2)) (>= i3 0))
                 (let [c3 (bit-and 0xff (.get in i3))]
                   (aset states 3 (update! ^SymbolState (aget ^objects (aget syms c3) l3) out' (aget states 3)))
                   (recur c3))
                 l3)))]
    (loop [l0 l0, l1 l1, l2 l2, l3 l3]
      (let [i0 (aget indices 0)]
        (if (>= i0 0)
          (let [c0 (bit-and 0xff (.get in i0))
                c1 (bit-and 0xff (.get in (aget indices 1)))
                c2 (bit-and 0xff (.get in (aget indices 2)))
                c3 (bit-and 0xff (.get in (aget indices 3)))]
            (aset states 3 (int (update! ^SymbolState (aget syms c3) out' (aget states 3))))
            (aset states 2 (int (update! ^SymbolState (aget syms c2) out' (aget states 2))))
            (aset states 1 (int (update! ^SymbolState (aget syms c1) out' (aget states 1))))
            (aset states 0 (int (update! ^SymbolState (aget syms c0) out' (aget states 0))))
            (recur c0 c1 c2 c3))
          (do (.putInt out' (aget states 3))
              (.putInt out' (aget states 2))
              (.putInt out' (aget states 1))
              (.putInt out' (aget states 0))))))
    (.flip out')
...
(defn encode
  ...
  (let [raw-size (.remaining in)
        ...
        _ (.rewind in)
        compressed-size (encode0* in syms out')]
