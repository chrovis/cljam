...
(defn- calc-level [levels {:keys [pos seq] :as alignment}]
  (let [segment [pos (+ pos (count seq))]
        level (or (some identity (map-indexed #(when-not (collide? segment %2) %1) levels))
                  (count levels))]
    [(assoc levels level segment) ;; state
     (update alignment :options conj {:LV {:type "i" :value level}}) ;; value
     ]))
  ...
  (let [filename (.getName (file (io/reader-path rdr)))
    ...
    (doseq [cache caches]
      (with-open [cache-rdr (bam/reader cache :ignore-index true)]
        (io/write-alignments wtr (io/read-alignments cache-rdr) hdr))
      (clean cache))))
